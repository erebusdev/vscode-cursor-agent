...
| Setting | Purpose |
| --- | --- |
| `cursorAcp.agentPath` | The `agent` executable or a wrapper script. Empty auto-detects `cursor-agent` (then `agent`) from PATH, your login shell, and `~/.local/bin`; a configured value always wins. |
